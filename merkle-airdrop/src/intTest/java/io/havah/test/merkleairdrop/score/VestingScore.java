...
public class VestingScore extends Score {
    ...
    public static VestingScore mustDeploy(TransactionHandler txHandler, Wallet wallet)
        ...
        String FILE_PATH = "../vesting/build/libs/vesting-1.0.0-optimized.jar";
        Score score = txHandler.deploy(wallet, FILE_PATH, params);
        LOG.infoExiting("scoreAddr = " + score.getAddress());
        return new VestingScore(score);
    }

    public TransactionResult setRewardManager(Wallet wallet, Address address) throws IOException, ResultTimeoutException {
        RpcObject.Builder params = new RpcObject.Builder()
                .put("_address", new RpcValue(address));
        return invokeAndWaitResult(wallet, "setRewardManager", params.build());
    }
    ...
    public TransactionResult registerMonthlyVesting(Wallet wallet, Address token, BigInteger startTime, BigInteger endTime,
        ...
        RpcObject params = new RpcObject.Builder()
                ...
                .put("_accounts", arr.build())
                .build();
        System.out.println("register startTime(" + startTime + "), endTime(" + endTime + ")");
...
//    public List<BigInteger> vestingTimes(int id) throws IOException {
//        RpcObject params = new RpcObject.Builder()
//                .put("_id", new RpcValue(BigInteger.valueOf(id)))
//                .build();
//
//        RpcArray array = call("vestingTimes", params).asArray();
//        List<RpcItem> list = array.asList();
//        return list.stream().map(RpcItem::asInteger).collect(Collectors.toList());
//    }
